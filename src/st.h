...
#define IS_CONTROL_C0(c) (BETWEEN((int)c, 0, 0x1f) || (c) == 0x7f)
#define IS_CONTROL_C1(c) (BETWEEN((int)c, 0x80, 0x9f))
...
#define ENUM_FIELDS \
    ...
    XX(ATTR_BOXDRAW)    \
    XX(ATTR_SIXEL)      \
    XX(ATTR_BOLD_FAINT, ATTR_BOLD | ATTR_FAINT)
#include "xenums.c"
...
#define ENUM_FIELDS \
    XX(WIN_MODE_VISIBLE) \
    XX(WIN_MODE_FOCUSED) \
    XX(WIN_MODE_APPKEYPAD) \
    XX(WIN_MODE_MOUSEBTN) \
    XX(WIN_MODE_MOUSEMOTION) \
    XX(WIN_MODE_REVERSE) \
    XX(WIN_MODE_KBDLOCK) \
    XX(WIN_MODE_HIDE) \
    XX(WIN_MODE_APPCURSOR) \
    XX(WIN_MODE_MOUSESGR) \
    XX(WIN_MODE_8BIT) \
    XX(WIN_MODE_BLINK) \
    XX(WIN_MODE_FBLINK) \
    XX(WIN_MODE_FOCUS) \
    XX(WIN_MODE_MOUSEX10) \
    XX(WIN_MODE_MOUSEMANY) \
    XX(WIN_MODE_BRCKTPASTE) \
    XX(WIN_MODE_NUMLOCK) \
    XX(WIN_MODE_MOUSE, WIN_MODE_MOUSEBTN    \
...
static int32 user_clipboard_copy_selection_image(Time time);
